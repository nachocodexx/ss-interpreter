import mx.cinvestav.domain.{BullyNode, BullyNodeEnvironments, ChordNode, ComposeFile, Consumer, DataContainer, DataPrepNode, DataPreparationEnvironments, DataPreparationNode, Deploy, Environments, Image, LB, LBEnvironments, Limits, LoadBalancer, LoadBalancing, Metadata, Network, Networks, Node, NodeId, PaxosNode, PaxosNodeEnvironments, Placement, PoolId, Port, Ports, RabbitMQNode, RabbitNode, Resources, SP, Service, Services, StorageNode, StorageNodeEnvironment, Volume, Volumes, buildArrayEnvs, buildRabbitNode, encoderComposeFile, encoderNetwork, encoderNetworks, encoderService, encoderStorageNode, encoderVolumes}
      ...
      val dcs = (0 until nodes).toList.map{ i=>
        ...
        DataContainer(
          ...
          environments = Environments(
            ...
            "CACHE_POLICY" -> "LFU",
            "IN_MEMORY" -> inMemory.toString,
            "STORAGE_PATH"-> storagePath.target,
          )
